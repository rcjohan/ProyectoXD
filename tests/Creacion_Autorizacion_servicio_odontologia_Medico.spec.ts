...
// Playwright
test('Creacion Autorizacion servicio Emergencia Medico', async ({ page }) => {
  const numAfiliado = await obtenerAfiliadoAleatorio();

  await page.goto('http://172.24.208.208:30129/login');

  await page.getByRole('textbox', { name: 'Usuario' }).fill('11020');
  await page.getByRole('textbox', { name: 'Usuario' }).press('Tab');
  await page.getByRole('textbox', { name: 'Contraseña' }).fill('11020');
  ...
  await page.getByRole('combobox').nth(1).selectOption('8'); // Odontologia
...
await page.getByRole('textbox', { name: 'Código de procedimiento' }).fill('1087');
